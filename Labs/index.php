...
if($result->num_rows > 0)
{
    ...
    while($row = $result->fetch_assoc())
    {
        ...
        echo "</td><td>\n";
        echo ('<a href="add.php?id'.htmlentities($row["ProductID"]).'">Add</a> /');
        echo ('<a href="view.php?id'.htmlentities($row["ProductID"]).'">View</a> /');
        echo ('<a href="update.php?id'.htmlentities($row["ProductID"]).'">Update</a> /');
